...
export default function EditTeacherScreen({ route, navigation }) {
  ...
  const [nome, setNome] = useState(teacher.nome);
  ...
  const handleSubmit = async () => {
    ...
    const payload = { nome: nome.trim(), email: email.trim(), role: 'teacher' };
    if (senha.trim()) payload.senha = senha;

    setLoading(true);
    try {
      await updateUser(teacher._id, payload);
      Alert.alert('Sucesso', 'Professor atualizado com sucesso!', [
        { text: 'OK', onPress: () => navigation.goBack() },
      ]);
    } catch (err) {
      Alert.alert('Erro', err?.response?.data?.error || 'Erro ao atualizar professor.');
    } finally {
      setLoading(false);
    }
  };
