...
export default function StudentListScreen({ navigation }) {
  ...
  const fetchStudents = useCallback(async (pageNum = 1, append = false) => {
    try {
      const data = await getUsers({ role: 'student', page: pageNum, limit: 10 });
      setStudents((prev) => (append ? [...prev, ...data.users] : data.users));
  ...
  const handleDelete = (id, nome) => {
    Alert.alert('Excluir Aluno', `Deseja excluir "${nome}"?`, [
